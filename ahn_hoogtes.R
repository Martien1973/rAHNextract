...
ahn_hoogtes <- function(name, X, Y, LONLAT = FALSE, AHN = "AHN3", resolution = 0.5, dem = "dsm", interpolated = TRUE, rm = FALSE, decimals = 2, type = "points"){
  ...
  my_url <- create_wcs_url(bbox = my_point$bbox, AHN = my_ahn, resolution = resolution, dem = dem, interpolated = interpolated)
...
#zet dem parameter op "dtm" (maaiveld) of "dsm" (ruw). Voor AHN1 is alleen dtm beschikbaar.
...
alle_hoogtes <- mapply(ahn_hoogtes, name = punten$ID, X = punten$X, Y = punten$Y, LONLAT = FALSE, AHN =  my_ahn, resolution = 0.5,  dem = my_dem, interpolated = TRUE, rm = TRUE, decimals = 2)
...
enkele_hoogte <- ahn_hoogtes(name = punten[my_ID, "ID"], X = punten[my_ID, "X"], Y = punten[my_ID, "Y"], LONLAT = FALSE, AHN =  "AHN2", resolution = 6, dem = "dtm", interpolated = TRUE, rm = TRUE)
